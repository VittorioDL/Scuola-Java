public class AlberoBT
{
    ...
    public NodoBT getNodo(NodoBT dato)
    {
        if(dato.compareTo(Radice)==0) return Radice;
        else return getNodo(dato, Radice);
    }
    
    private NodoBT getNodo(NodoBT dato, NodoBT r)
    {
        if(r == null) return null;
        if(dato.compareTo(r)==0) return r;
        if(dato.compareTo(r)>0) return getNodo(dato, r.getDestra());
        else return getNodo(dato, r.getSinistra());
    }
